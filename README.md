...
word of each command, as described in
...
described in [Commands](#commands) and [Pipelines](#pipelines). While these
structures are being built, shell expansions are applied to each word token,
as described in [Shell Expansions](#shell-expansions), producing the final
command names, arguments, and redirection targets. Once a complete pipeline
has been parsed, it is passed to the execution phase.
...
Builtins are executed directly within the shell process, without creating a new
process, so that their effects (such as variable assignments or directory
changes) take effect in the current shell environment.
...
If the executable is not found, the shell attempts to execute it relative to the
current working directory.

Commands that are part of the same pipeline run concurrently, each in its own
process, with pipes connecting their standard input and output streams. The
