...
deeper into systems programming and understand how a real shell works.  
...
- **Full command language**: Supports commands, pipelines, background execution,
  I/O redirections, quoting, escape sequences, and comments.
- **Internal variables and environment**: Maintains an internal hash map of shell
  and environment variables, and provides builtins to define, update, and remove
  them.
...
- **Interactive mode**: Provides a full interactive shell experience with a
  dynamic prompt, line editing, command history, and context-aware tab completion.
- **Job control**: Supports foreground and background job management, including
  listing, suspending, resuming, and signaling jobs.
- **Scripts**: Allows executing script files and sourcing them into the
  current shell environment.
- **Startup**: Supports automatic initialization for login and non-login shells by
  setting up a default environment and allowing users to customize startup files.
...
- [🔤 2 Shell Language](#shell-language)
    ...
    - [2.4 Quoting and Escaping](#quoting-and-escaping)
        - [2.4.1 Single Quotes](#single-quotes)
        - [2.4.2 Double Quotes](#double-quotes)
        - [2.4.3 Escape Sequences](#escape-sequences)
        - [2.4.4 Line Continuation](#line-continuation)
    - [2.5 Redirections](#redirections)
...
Below are example install commands for Debian/Ubuntu-based systems.
...
> ℹ️ **Note:** `xd-shell` requires **GCC** and **GNU Make** specifically. On
> Debian/Ubuntu, GNU Make is invoked as `make`. On systems where GNU Make is
> installed as `gmake`, use `gmake` instead.

--- 

### 1.2 Building from Source <a name="building-from-source"></a>

Clone the repository, then build the project as follows:
...
This produces the `xd_shell` binary in the `bin/` directory. You can run it
...
By default, this installs the `xd_shell` binary into `/usr/local/bin`.

> ℹ️ **Note:** If you want to remove the installation later, run: 
> `sudo make uninstall`. 
...
A script file may be specified as an argument to execute commands from that file
(see [Running Scripts](#running-scripts)), or the `-c` option may be used to
execute commands provided as a string.
...
A command consists of a command name (a builtin or external program), zero or 
more arguments, and optional I/O redirections (see
...
the next command using the pipe character (`|`).
...
### 2.3 Comments <a name="comments"></a>

A comment begins with the `#` character and continues to the end of the line. 
It is recognized only when `#` appears outside of single quotes `'...'` or 
double quotes `"..."`. Everything from `#` up to the next newline is ignored 
by the shell.

---

### 2.4 Quoting and Escaping <a name="quoting-and-escaping"></a>
...
#### 2.4.1 Single Quotes <a name="single-quotes"></a>
...
#### 2.4.2 Double Quotes <a name="double-quotes"></a>
...
#### 2.4.3 Escape Sequences <a name="escape-sequences"></a>
...
#### 2.4.4 Line Continuation <a name="line-continuation"></a>
...
### 2.5 Redirections <a name="redirections"></a>
...
After input has been read, the shell processes it in three main phases:
scanning, parsing, and execution.

During the scanning phase, the input stream is tokenized. Alias expansion is
applied to the command word of each command, as described in
[Alias Expansion](#alias-expansion). The resulting token stream is then passed
to the parser.

During the parsing phase, the token stream is parsed into command and pipeline
structures, as defined in [Commands](#commands) and [Pipelines](#pipelines).
As these structures are constructed, shell expansions are applied to each
argument token as it is parsed, so that commands are built using the expanded
arguments (see [Shell Expansions](#shell-expansions)). Redirection targets are
also expanded during parsing (see [Redirections](#redirections)). Once a 
complete pipeline has been parsed, it is passed to the execution phase.

During execution, any redirections associated with each command are applied to
establish the appropriate standard input, output, and error streams. Each
command is then resolved as either a shell builtin or an external command. For
external commands, if the command name contains a `/`, it is treated as an
explicit path and executed directly. Otherwise, the shell searches the
directories listed in the `$PATH` environment variable to locate the executable
and runs it.

Commands in a pipeline run concurrently, each in a separate process. The exit
status of a pipeline is the exit status of its last command.

When running interactively, an executed pipeline is additionally treated as a
job, and job control and terminal control behavior applies, as described in
...
Scripts can be executed in two ways: by invoking `xd-shell` with a script file,
or by running an executable script as a normal command.

When `xd-shell` is invoked with a script file argument, it reads commands from
that file, executes them sequentially, and then exits.

When an executable script is run as a normal command, `xd-shell` resolves the
command name using the same rules as other external commands: if the name
contains a `/`, it is treated as an explicit path and executed directly.
Otherwise, it is searched for in the directories listed in `$PATH`.

Before executing a script file, `xd-shell` checks whether it appears to be a
valid text file. Files identified as binaries are not executed as shell scripts.
If the target is executable but not a native binary, `xd-shell` falls back to
running it as a script by starting a new shell instance and using the script
path as its input source. If the script begins with a `#!` interpreter line, the
operating system may instead invoke the specified interpreter directly.

> ℹ️ **Note:** When running a script as a command, `xd-shell` executes it in a
> separate shell process. To run a script in the current shell environment,
> use [The `source` builtin](#the-source-builtin).
